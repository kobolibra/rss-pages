#!/usr/bin/env python3
"""Post-process KKR Insights local item pages.

Rehosts remote images (kkr.com CDN) onto our own Pages site so reading apps
such as Readwise Reader can fetch them reliably. Already-local images and
non-image content are left untouched.

Usage: python fix_kkr_pages.py <site_dir> [base_url]
"""
...
FEED_NAME = "kkr_insights"

HEADERS = {
    "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/136.0.0.0 Safari/537.36",
    "Referer": "https://www.kkr.com/",
}
...
def rehost_remote_images(text: str, out_dir: Path) -> str:
    ...
    for src in remote:
        ...
        try:
            ...
            print("[fix_kkr] rehosted {} -> {}".format(real_url, out_dir / fname))
        except Exception as exc:
            print("[fix_kkr] keep remote (fetch failed) {}: {}".format(src, exc))
    return text
...
def recover_local_images(text: str, out_dir: Path, asset_base: str) -> None:
    ...
    for src in IMG_SRC_RE.findall(text):
        ...
        try:
            ...
            print("[fix_kkr] recovered missing asset {} -> {}".format(url, local))
        except Exception as exc:
            print("[fix_kkr] could not recover {}: {}".format(url, exc))
...
def process_file(path: Path, site_dir: Path, base_url: str) -> None:
    ...
    if new_text != text:
        ...
        print("[fix_kkr] updated {}".format(path))
...
def main() -> int:
    if len(sys.argv) < 2:
        print("Usage: python fix_kkr_pages.py <site_dir> [base_url]", file=sys.stderr)
        return 2
    site_dir = Path(sys.argv[1])
    base_url = sys.argv[2].rstrip("/") if len(sys.argv) > 2 else ""
    item_root = site_dir / "item" / FEED_NAME
    if not item_root.exists():
        print("[fix_kkr] no kkr item dir at {}; nothing to do".format(item_root))
        return 0
